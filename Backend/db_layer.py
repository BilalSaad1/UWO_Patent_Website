...
def search_patents(
    ...
    """
    Search inactive patents by title.
    ...
    with Session(engine) as s:
        if tokens:
            cond = and_(
                *[InactivePatent.title.ilike(f"%{t}%") for t in tokens]
            )
        else:
            cond = text("TRUE")
        ...
        if year_from:
            cond = cond & (
                InactivePatent.grant_date
                >= func.to_date(f"{year_from}-01-01", "YYYY-MM-DD")
            )
        if year_to:
            cond = cond & (
                InactivePatent.grant_date
                <= func.to_date(f"{year_to}-12-31", "YYYY-MM-DD")
            )
        ...
        if (sort_by or "").lower() == "title":
            ...
            if (sort_dir or "").lower() == "asc":
                order_by = [
                    order_expr.asc(),
                    InactivePatent.grant_date.desc().nullslast(),
                ]
            else:
                order_by = [
                    order_expr.desc(),
            ...
            if (sort_dir or "").lower() == "asc":
                order_by = [
                    InactivePatent.grant_date.asc().nullsfirst(),
                    func.lower(InactivePatent.title).asc(),
                ]
            else:
                order_by = [
                    InactivePatent.grant_date.desc().nullslast(),
